...
from uchan.lib.model import BoardModel, ModeratorLogType
...
class AddBoardForm(CSRFForm):
    ...
    board_name = StringField(
        "Name",
        [DataRequired(), BoardNameValidator()],
        description="Name of the board. This name is used in the url and cannot be "
        "changed, so choose carefully. You can have a maximum of ("
        + str(config.max_boards_per_moderator)
        + ") boards.",
    )
...
class BoardConfigurationForm(CSRFForm):
    ...
    max_files = IntegerField(
        "Max files",
        [DataRequired(), NumberRange(min=1, max=validation.MAX_FILES)],
        default=3,
...
def mod_boards():
    ...
    board_moderators = moderator_service.get_all_board_moderators_by_moderator(
        moderator
    )
...
def mod_board(board_name):
    ...
    # These are purely for configuring the visibility of the various elements on the
    # page, the actions are still checked with the authorizer on post.
    can_update_board_config = moderator_service.can_update_board_config(
        moderator, board
    )
    can_update_advanced_board_configs = (
        moderator_service.can_update_advanced_board_configs(moderator)
    )
        ...
        if action_configure:
            ...
            if board_configuration_form.validate():
                ...
                if can_update_advanced_board_configs:
                    ...
                    board.config.file_posting = (
                        board_configuration_form.file_posting.data
                    )
                    board.config.posting_verification_required = (
                        board_configuration_form.posting_verification.data
                    )
            ...
            try:
                moderator_service.user_update_roles(
                    moderator, board, moderator_username, checked_roles
                )
    ...
    invite_moderator_form.action_url = url_for(
        ".mod_board", board_name=board_name, _anchor="invite"
    )
    ...
    board_moderators_unsorted = sorted(
        board_moderators, key=lambda board_moderator: board_moderator.moderator.id
    )
    board_moderators = []
