...
class ChangePasswordForm(CSRFForm):
    ...
    old_password = PasswordField(
        "Old password", [DataRequired(), ModeratorPasswordValidator()]
    )
    new_password = PasswordField(
        "New password", [DataRequired(), ModeratorPasswordValidator()]
    )
...
def mod_self():
    ...
    board_links = map(
        lambda b: (b.name, url_for("board", board_name=b.name)), moderating_boards
    )
