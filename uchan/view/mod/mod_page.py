...
from uchan.view.form.validators import PageLinkValidator, PageTitleValidator
...
class AddPageForm(CSRFForm):
    ...
    title = StringField(
        "Title",
        [DataRequired(), PageTitleValidator(), Length(max=validation.TITLE_MAX_LENGTH)],
    )
    link = StringField(
        "Link name",
        [
            DataRequired(),
            PageLinkValidator(),
            Length(max=validation.LINK_NAME_MAX_LENGTH),
        ],
    )
...
class ModifyPageForm(CSRFForm):
    ...
    order = IntegerField(
        ...
        [NumberRange(min=0)],
        default=0,
        description="Order of the page, where applicable",
    )
    content = TextAreaField(
        "Content",
        [Length(max=validation.CONTENT_MAX_LENGTH)],
        default="",
...
def mod_page(page: PageModel):
    ...
    if not modify_page_form:
        modify_page_form = ModifyPageForm(
            title=page.title, order=page.order, content=page.content
        )
    ...
    return render_template(
        "mod_page.html",
        page=page,
        modify_page_form=modify_page_form,
