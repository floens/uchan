...
class Recaptcha2Method(verification_service.VerificationMethod):
    def __init__(self, sitekey, secret):
        ...
        """.replace(
            "__sitekey__", self.sitekey
        )

        self.javascript = """
        <script src="https://www.google.com/recaptcha/api.js?onload=g_captcha2_callback\
    ...
    def verify_request(self, request):
        ...
        try:
            ...
            raise ArgumentError("Error contacting recaptcha service") from e
...
def on_enable():
    sitekey = os.getenv(
        "GOOGLE_CAPTCHA2_SITEKEY", "6LeIxAcTAAAAAJcZVRqyHh71UMIEGNQ_MXjiZKhI"
    )
    secret = os.getenv(
        "GOOGLE_CAPTCHA2_SECRET", "6LeIxAcTAAAAAGG-vFI1TnRWxMZNFuojJ4WifJWe"
    )

    if not sitekey or not secret:
        raise RuntimeError(
            "Required keys not found in the environment. "
            "Please set the GOOGLE_CAPTCHA2_SITEKEY and GOOGLE_CAPTCHA2_SECRET"
