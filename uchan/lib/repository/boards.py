...
from uchan.lib.model import BoardConfigModel, BoardModel, PostModel, ThreadModel
from uchan.lib.ormmodel import BoardOrmModel, PostOrmModel, ThreadOrmModel
    ...
    if all_board_names_cached is not None:
        ...
        with session() as s:
            q = (
                ...
                .options(load_only(BoardOrmModel.name))
                .order_by(BoardOrmModel.name)
            )
...
def get_all_boards_with_last_threads(
    ...
    with session() as s:
        q = (
            s.query(BoardOrmModel, ThreadOrmModel, PostOrmModel)
            .filter(
                BoardOrmModel.id == ThreadOrmModel.board_id,
        ...
        res = list(
            map(
                lambda i: (
                    BoardModel.from_orm_model(i[0]),
                    ThreadModel.from_orm_model(i[1]),
                    PostModel.from_orm_model(i[2]),
                ),
                b,
            )
        )
    ...
    with session() as s:
        for name in names:
            ...
            if board_cache:
                ...
                board_orm_model = (
                    s.query(BoardOrmModel).filter_by(name=name).one_or_none()
                )
                if board_orm_model:
                    board = BoardModel.from_orm_model(
                        board_orm_model, include_config=True
                    )
                    cache.set(cache_key("board_and_config", name), board.to_cache())
                    boards.append(board)
...
def delete(board: BoardModel):
    with session() as s:
        ...
        # The pages etc. will fall out of the cache themselves. This is the first thing
        # all board related endpoints use, so they get cancelled at the start of the
        # request. If any are still working with caches of this board let them use the
        # leftover caches.
...
def _set_all_board_names_cache(s):
    ...
    cache.set(
        cache_key("all_board_names"),
        list(map(lambda i: i.name, all_board_names_q.all())),
    )
