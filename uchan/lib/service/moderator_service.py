...
from uchan.lib.repository import board_moderators, boards, moderator_logs, moderators
...
def check_and_set_password(
    moderator: ModeratorModel, old_password: str, new_password: str
):
...
def can_update_board_config(moderator: ModeratorModel, board: BoardModel) -> bool:
    try:
        action_authorizer.authorize_board_action(
            moderator, board, ModeratorBoardAction.CONFIG_UPDATE
        )
        return True
...
def can_update_roles(moderator: ModeratorModel, board: BoardModel) -> bool:
    try:
        action_authorizer.authorize_board_action(
            moderator, board, ModeratorBoardAction.ROLES_UPDATE
        )
        return True
    except NoPermissionError:
        return False
...
def can_invite_moderator(moderator: ModeratorModel, board: BoardModel):
    try:
        action_authorizer.authorize_board_action(
            moderator, board, ModeratorBoardAction.MODERATOR_ADD
        )
        return True
    except NoPermissionError:
        return False
...
def can_remove_moderator(moderator: ModeratorModel, board: BoardModel):
    try:
        action_authorizer.authorize_board_action(
            moderator, board, ModeratorBoardAction.MODERATOR_REMOVE
        )
        return True
    except NoPermissionError:
        return False
...
def user_update_board_config(moderator: ModeratorModel, board: BoardModel):
    action_authorizer.authorize_board_action(
        moderator, board, action_authorizer.ModeratorBoardAction.CONFIG_UPDATE
    )
...
def user_invite_moderator(moderator: ModeratorModel, board: BoardModel, username: str):
    ...
    log(
        ModeratorLogType.MODERATOR_INVITE,
        moderator,
        board,
...
def user_remove_moderator(moderator: ModeratorModel, board: BoardModel, username: str):
    ...
    if moderator.id == member.id:
        ...
        action_authorizer.authorize_board_action(
            moderator, board, action_authorizer.ModeratorBoardAction.MODERATOR_REMOVE
        )
        board_service.remove_moderator(board, member)
        log(
            ModeratorLogType.MODERATOR_REMOVE,
            moderator,
            board,
...
def user_update_roles(
    moderator: ModeratorModel, board: BoardModel, username: str, new_roles: "List[str]"
):
    action_authorizer.authorize_board_action(
    ...
    if changed:
        ...
        for add in added_roles:
            ...
            log(
                ModeratorLogType.MODERATOR_ROLE_ADD,
                moderator,
                board,
        ...
        for remove in removed_roles:
            ...
            log(
                ModeratorLogType.MODERATOR_ROLE_REMOVE,
                moderator,
                board,
...
def user_register(username: str, password: str, password_repeat: str):
    """
    Register a moderator with the given passwords. The created moderator has no roles
    and no relationships to boards. If user registration was disabled in the site
    configuration, this methods throws an ArgumentError.
    ...
    :raises ArgumentError any error defined in
    :meth:`uchan.lib.repository.moderators.create_with_password`
...
def user_get_logs(
    moderator: ModeratorModel, board: BoardModel, page: int, per_page: int
) -> "List[ModeratorLogModel]":
    action_authorizer.authorize_board_action(
        moderator, board, action_authorizer.ModeratorBoardAction.VIEW_LOG
    )
...
def log(
    log_type: ModeratorLogType, moderator: ModeratorModel, board: BoardModel, text: str
):
