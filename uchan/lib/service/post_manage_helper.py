...
from uchan.lib.model import ModeratorModel, PostModel, ThreadModel
...
def handle_manage_post(details: ManagePostDetails):
    thread = posts.find_thread_by_board_name_thread_refno(
        details.board_name, details.thread_refno
    )
    if not thread:
    ...
    if (
        ...
        or details.mode == ManagePostDetails.REPORT
    ):
        _manage_post(details, moderator)
    elif (
        details.mode == ManagePostDetails.TOGGLE_STICKY
        or details.mode == ManagePostDetails.TOGGLE_LOCKED
    ):
...
def _manage_post(details: ManagePostDetails, moderator: ModeratorModel):
    ...
    if not post:
        raise BadRequestError(
            MESSAGE_NO_POST_ID if not details.post_id else MESSAGE_POST_NOT_FOUND
        )
...
def _manage_delete(
    details: ManagePostDetails, moderator: ModeratorModel, post: PostModel
):
    try:
        action_authorizer.authorize_post_action(
            moderator, PostAction.POST_DELETE, post, details
        )
        ...
        raise BadRequestError(MESSAGE_DELETE_NO_PERMISSION) from e
...
def _manage_report(
    ...
    action_authorizer.authorize_post_action(
        moderator, PostAction.POST_REPORT, post, details
    )
...
def _manage_thread(
    thread: ThreadModel, details: ManagePostDetails, moderator: ModeratorModel
):
    if not moderator:
...
def _manage_sticky_toggle(
    thread: ThreadModel, details: ManagePostDetails, moderator: ModeratorModel
):
    action_authorizer.authorize_post_action(
        moderator, PostAction.THREAD_STICKY_TOGGLE, board=thread.board
    )

    posts.update_thread_sticky(thread, not thread.sticky)

    message = "sticky on /{}/{} {}".format(
        thread.board.name, thread.id, "disabled" if thread.sticky else "enabled"
    )
    mod_log(message, ip4_str=ip4_to_str(details.ip4), moderator=moderator)


def _manage_locked_toggle(
    thread: ThreadModel, details: ManagePostDetails, moderator: ModeratorModel
):
    action_authorizer.authorize_post_action(
        moderator, PostAction.THREAD_LOCKED_TOGGLE, board=thread.board
    )

    posts.update_thread_locked(thread, not thread.locked)

    message = "lock on /{}/{} {}".format(
