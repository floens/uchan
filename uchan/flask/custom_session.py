...
class CustomSession(CallbackDict, SessionMixin):
    def __init__(
        self, initial=None, session_id=None, new=False, was_invalid=False, expires=0
    ):
        def on_update(self):
            self.modified = True
...
class CustomSessionInterface(SessionInterface):
    ...
    def open_session(self, app, request):
        ...
        expires = int(
            now()
            + 1000 * datetime.timedelta(minutes=self.EXPIRES_MINUTES).total_seconds()
        )
        if not session_id:
            # No session, generate one for the client
            return self.session_class(
                session_id=self.generate_session_id(), new=True, expires=expires
            )
        else:
        ...
        # Invalid session id or expired, generate a new one Force so that it will always
        # be processed in save session, either to be deleted or set
        return self.session_class(
    ...
    def save_session(self, app, session, response):
        ...
        if session.was_invalid:
            ...
            if not session:
                ...
                # Was invalid is only for new sessions, so no db and cache synching
                # needed
            ...
            if not session:
                ...
                # there's no point deleting a cookie when it was never saved on the
    ...
    def find_session_for_id(self, session_id):
        ...
        if cache_data is not None:
            ...
            with sessioncontext() as s:
                try:
                    session_model = (
                        s.query(SessionOrmModel).filter_by(session_id=session_id).one()
                    )

                    session = CustomSession(
                        initial=session_model.data,
                        session_id=session_id,
    ...
    def store_cookie(self, app, session, response):
        ...
        # datetime.timedelta(minutes=self.EXPIRES_MINUTES)
    ...
    def delete_cookie(self, app, session, response):
        response.delete_cookie(
            app.config["SESSION_COOKIE_NAME"], domain=self.get_cookie_domain(app)
        )

    def store_session_db(self, session):
        session_model = SessionOrmModel(
            session_id=session.session_id, data=session, expires=session.expires
        )
    ...
    def store_session_cache(self, session):
        self.cache.set(
            ...
            CustomSessionCacheDict(session, session.expires),
        )
